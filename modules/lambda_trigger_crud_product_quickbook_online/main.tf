...
resource "aws_cloudwatch_event_rule" "update_inventory_event" {
  name                = "${var.environment_name}_${var.trigger_crud_product_quickbook_online_name}"
  schedule_expression = "cron(0 8,18 * * ? *)"
}

resource "aws_cloudwatch_event_target" "crud_product_sqs" {
  rule      = aws_cloudwatch_event_rule.update_inventory_event.name
  target_id = "SendToSQS"
  arn       = aws_sqs_queue.crud_product_sqs.arn
}
