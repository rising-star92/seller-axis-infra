...
resource "aws_sqs_queue" "failure_update_inventory_sqs" {
  name = "${var.environment_name}-failure-${var.update_inventory_sqs_name}"
  tags = {
...
resource "aws_sqs_queue" "update_inventory_sqs" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.failure_update_inventory_sqs.arn,
    maxReceiveCount     = 3
  })
  tags = {
    Environment = var.environment_name
